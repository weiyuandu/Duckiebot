...
def calculate_distance_to_point(x_curr, y_curr, x_target, y_target):
    """
    ...
    -------
    float
        Distance to target point in meters
    """
    return np.sqrt((x_target - x_curr)**2 + (y_target - y_curr)**2)


def calculate_angle_to_point(x_curr, y_curr, x_target, y_target):
    """
    ...
    -------
    float
        Angle to target point in radians [0, 2π)
    """
    angle = np.arctan2(y_target - y_curr, x_target - x_curr)
    if angle < 0:
        angle += 2 * np.pi
    return angle
