...
def make_train_env(map_name: str):
    # Training environment: with randomization to improve robustness
...
def make_eval_env(map_name: str):
    # Evaluation environment: fixed, not randomized, to facilitate comparison of trends and different runs
    return DuckieAvoidWrapper(
        map_name=map_name,
        obs_size=(84, 84),
        frame_skip=2,
...
def plot_training_curves(monitor_path: str, eval_npz_path: str, out_png: str):
    # monitor.csv: written by VecMonitor, # at the beginning of the line is a comment
    ...
    # Cumulative timesteps (the length of each episode is accumulated)
    df["timesteps"] = df["l"].cumsum()
    # Adaptive sliding average window (divided into approximately 50 segments)
    window = max(1, len(df) // 50)
    df["reward_ma"] = df["r"].rolling(window=window, min_periods=1).mean()

    # Cycle evaluation (written by EvalCallback)
...
def main():
    ...
    # record in CSV + TensorBoard
    new_logger = configure(folder=log_dir, format_strings=["stdout", "csv", "tensorboard"])
    model.set_logger(new_logger)

    # Evaluation callback (write evaluations.npz & save the best model)
